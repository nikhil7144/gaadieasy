import { createPendingSellerSignup, getPendingSellerSignup, updatePendingSellerSignupEmail } from "@/lib/services/seller-auth";
import { sellerSignupChangeEmailSchema, sellerSignupStartSchema } from "@/lib/validations/seller";
...
export async function GET(request: Request) {
  ...
  try {
    const pending = await getPendingSellerSignup(pendingId);
...
export async function POST(request: Request) {
  const parsed = sellerSignupStartSchema.safeParse(await request.json());
  if (!parsed.success) return Response.json({ error: "Business name and a valid email are required" }, { status: 400 });

  try {
    const origin = new URL(request.url).origin;
    const pending = await createPendingSellerSignup(parsed.data.businessName, parsed.data.email, origin);
...
export async function PATCH(request: Request) {
  const parsed = sellerSignupChangeEmailSchema.safeParse(await request.json());
  if (!parsed.success) return Response.json({ error: "A valid email is required" }, { status: 400 });

  try {
    const origin = new URL(request.url).origin;
    const pending = await updatePendingSellerSignupEmail(parsed.data.pendingId, parsed.data.email, origin);
    return Response.json(pending);
  } catch (error) {
