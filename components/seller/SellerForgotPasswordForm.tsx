...
export function SellerForgotPasswordForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const { error: resetError } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${window.location.origin}/gaadigear/sell/reset-password`,
    });
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <label className="mt-6 block text-sm font-bold text-slate-700" htmlFor="forgot-email">
        Email
      </label>
      <input
      ...
      <p className="mt-4 text-center text-sm text-slate-500">
        <a className="font-bold text-emerald-700 hover:underline" href="/gaadigear/sell/login">
          Back to sign in
        </a>
      </p>
    </form>
